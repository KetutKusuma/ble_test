...
class Command {
  ...
  Future<BLEResponse<List<int>>> handshake(
    ...
    try {
      ...
      log("response write handshake : ${responseWrite}");

      return BLEResponse.success("Sukses handshake",
          data: responseWrite.buffer);
    } catch (e) {
  ...
  Future<BLEResponse> login(BluetoothDevice device, BLEProvider bleProvider,
    ...
    try {
      ...
      List<int> resultAES256 = CryptoUtilsV2.aesEncrypt(
      ...
      if (!resultaddArray) {
        log("error when add array uint8");
        return BLEResponse.error("Error when add array uint8");
      }

      List<int> data = MessageV2()
          .createEnd(0, buffer, InitConfig.data().KEY, InitConfig.data().IV);
  ...
  Future<BLEResponse<AdminModels>> getAdminData(
    ...
    try {
      ...
      // if (params.length != 6) {
      //   throw Exception("Retrieve parameter expected 6 parameters");
      // }
