...
class _LogExplorerScreenState extends State<LogExplorerScreen> {
  ...
  initGetLogExplorer() async {
    try {
      BLEResponse<ToppiExplorerModel> data = await commandImageFile
          .getLogExplorer(bleProvider, 255);
      if (!data.status) {
        Snackbar.show(
      ...
      BLEResponse<List<int>> dataBuffer = await commandImageFile
          .dataBufferTransmit(bleProvider, toppiFile, 255);
  ...
  Future getBufferDataLogFile(List<int> fileName, String fileNameString) async {
    try {
      ...
      BLEResponse<ToppiFileModel> data = await commandImageFile
          .logFilePrepareTransmit(bleProvider, fileName, 255);
      if (!data.status) {
        _progressDialog.hide();
      ...
      Uint8List dataGzip = await decompressGzip(
        Uint8List.fromList(dataBuffer.data ?? []),
      );
  ...
  Widget build(BuildContext context) {
    return ScaffoldMessenger(
      ...
      child: Scaffold(
        appBar: AppBar(elevation: 0, title: const Text("Daftar Catatan")),
        body: SmartRefresher(
              ...
              SliverList(
                delegate: SliverChildBuilderDelegate((context, index) {
                  return GestureDetector(
                    onTap: () {
                      getBufferDataLogFile(
                        listLogExplorer[index].filename,
